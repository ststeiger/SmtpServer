...
namespace SyslogNet.Client
{
    ...
    public class SyslogOptions
    {
        ...
        {
            // / UPD: 514 , TCP: 601, TLS: 6514
            if (this.SyslogServerPort == 514)
                this.NetworkProtocol = NetworkProtocols.UPD;

            if (this.SyslogServerPort == 6514)
                this.NetworkProtocol = NetworkProtocols.TLS;

            this.NetworkProtocol = NetworkProtocols.TCP;
        }
        ...
        public void InferDefaultPort()
        {
            ...
            switch (this.NetworkProtocol)
            {
                case NetworkProtocols.TCP:
                    this.SyslogServerPort = 601; // was 1468; // TCP 
                    break;
